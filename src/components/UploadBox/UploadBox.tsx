...
interface Props {
  file: PrizeType[];
  setFile: React.Dispatch<React.SetStateAction<PrizeType[]>>;
  idx: number;
  disabled: boolean;
}

export const UploadBox = ({ idx, setFile, file, disabled }: Props) => {
  ...
  const [uploadImage, { loading: uploadLoading }] =
    useMutation<UploadGiveawayImageResponse>(UPLOAD_GIVEAWAY_IMAGE, {
      onCompleted: (data) => {
        setFile((prev) => {
          prev[idx].image = data.uploadGiveawayImage;
          return [...prev];
        });
      },
      onError: (e) => {
        ...
        fileList: [
          {
            ...
            name: file[idx].image,
            status: 'done',
            url: `${process.env.REACT_APP_GIVEAWAY_URL}/${file[idx].image}`,
          },
        ],
      });
    }
